...
def inline(message):
    ...
    if message.text == 'Случайный' or message.text == 'По новой "Рандом"':
        getScheduleRandom(getFilmRandom(message), message)
    if message.text == 'Из топа' or message.text == 'По новой "Из Топа"':
        getFilmTop(message)
...
def getScheduleRandom(Web_page, message):
    ...
    bot.send_message(message.chat.id, film_text, reply_markup=key, parse_mode='HTML')


def getFilmRandom(message):
